...
def run(skip_api: bool = False) -> None:
    ...
    if len(hist_scaled) < 72:
        raise RuntimeError("학습 CSV 길이가 72 미만이라 윈도우 구성 불가")

    if skip_api:
        print("[warn] --skip-api: API 호출 없이 학습 CSV 마지막 행으로 테스트")
        row_scaled = hist_scaled.iloc[[-1]][feat_cols + ["시간"]].copy()
        bucket = requested_bucket
        row_scaled["시간"] = bucket.strftime("%Y-%m-%d %H:%M:%S")
        ...
        row_scaled, bucket, debug = build_realtime_feature_row(hist_scaled, env, requested_bucket)

    window = pd.concat([hist_scaled.iloc[-71:].copy(), row_scaled.copy()], ignore_index=True)
